...
// calcBMI test PASSED: console.log(calcBMI(16.9, 105.4))
  ...
  return (
    <Card>
      ...
      <Card.Description>BMI: {calcBMI(props.source.mass, props.source.height)}</Card.Description>
      </Card.Content>
      <Card.Content extra><p>Appears in: </p><ul>{props.source.films.map(i => <Films films={i} key={i} />)}</ul></Card.Content>
    </Card>
  );
}
